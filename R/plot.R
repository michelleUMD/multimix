plot_drug_probabilities <- function(est_list, tgrid = seq(0.01, 12*5, length.out = 400),
  ...
  ggplot() +
    ...
    scale_x_continuous(name = "Time (months)", limits = c(0.03, 60)) +
    labs(title = title) +
...
plot_drug_probabilities_lite <- function(est_list, tgrid = seq(0.03, 12*5, length.out = 400),
  ...
  ggplot() +
    ...
    geom_line(data = avg_ci_df, aes(x = Time, y = Average_Fitted),
              color = "blue", size = 1) +
    scale_y_continuous(name = "Probability", limits = c(0, 1)) +
    scale_x_continuous(name = "Probability", limits = c(0.03, 60)) +
    labs(x = "Time (months)", title = title) +
    theme_minimal(base_size = 14)
}
