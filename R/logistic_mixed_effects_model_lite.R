source("/studies/cardiac/pericardium/recurrent_pericarditis/radical_pericardiectomy/analyses/time_functions.R")
source("/studies/cardiac/pericardium/recurrent_pericarditis/radical_pericardiectomy/analyses/plot_functions.R")
...
for (drug_class in drug_classes_of_interest) {
  ...
  est_list <- fit_model_with_retries_lite(
...
plot_all_drug_classes_facet(estimated_param_list, max_time = 144)
...
loglik_long <- data.frame(
  model = names(estimated_param_list),
...
est_long <- bind_rows(est_long, loglik_long)
